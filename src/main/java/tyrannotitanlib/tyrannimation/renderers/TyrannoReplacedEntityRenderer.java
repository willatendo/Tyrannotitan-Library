...
public abstract class TyrannoReplacedEntityRenderer<T extends IAnimated> extends EntityRenderer implements IAnimatedRenderer 
{
	private final AnimatedModel<IAnimated> modelProvider;
	private final T animatable;
	protected final List<AnimatedLayerRenderer> layerRenderers = Lists.newArrayList();
	private IAnimated currentAnimatable;
	private static Map<Class<? extends IAnimated>, TyrannoReplacedEntityRenderer> renderers = new ConcurrentHashMap<>();
	...
	protected TyrannoReplacedEntityRenderer(EntityRendererProvider.Context renderManager, AnimatedModel<IAnimated> modelProvider, T animatable) 
	{
		super(renderManager);
	...
	public static void registerReplacedEntity(Class<? extends IAnimated> itemClass, TyrannoReplacedEntityRenderer renderer) 
	{
		renderers.put(itemClass, renderer);
	}

	public static TyrannoReplacedEntityRenderer getRenderer(Class<? extends IAnimated> item) 
	{
	...
	public void render(Entity entity, IAnimated animatable, float entityYaw, float partialTicks, PoseStack stack, MultiBufferSource bufferIn, int packedLightIn) 
	{
		...
		{
			((IAnimatedModel) modelProvider).setLivingAnimations(animatable, this.getUniqueID(entity), predicate);
		}
		...
		{
			for(AnimatedLayerRenderer layerRenderer : this.layerRenderers) 
			{
	...
	@Override
	public AnimatedModel getAnimatedModelProvider() 
	{
	...
	public final boolean addLayer(AnimatedLayerRenderer<? extends LivingEntity> layer) 
	{
