...
public abstract class AnimatedEntityRenderer<T extends LivingEntity & IAnimated> extends EntityRenderer<T> implements IAnimatedRenderer<T> {
	static {
		AnimationController.addModelFetcher((IAnimated object) -> {
			if (object instanceof Entity) {
				return (IAnimatedModel<Object>) AnimationUtilities.getTyrannoModelForEntity((Entity) object);
			}
			return null;
		});
	}
	...
	protected final List<AnimatedLayerRenderer<T>> layerRenderers = Lists.newArrayList();
	...
	protected AnimatedEntityRenderer(EntityRendererProvider.Context renderManager, AnimatedModel<T> modelProvider) {
		super(renderManager);
	...
	public void render(T entity, float entityYaw, float partialTicks, PoseStack stack, MultiBufferSource bufferIn, int packedLightIn) {
		...
		if (!entity.isSpectator()) {
			for (AnimatedLayerRenderer<T> layerRenderer : this.layerRenderers) {
	...
	public void renderEarly(T animatable, PoseStack stackIn, float ticks, MultiBufferSource renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float partialTicks) {
		...
		IAnimatedRenderer.super.renderEarly(animatable, stackIn, ticks, renderTypeBuffer, vertexBuilder, packedLightIn, packedOverlayIn, red, green, blue, partialTicks);
	}
	...
	public final boolean addLayer(AnimatedLayerRenderer<T> layer) {
