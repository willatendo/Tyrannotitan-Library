...
public class TyrannobookScreen extends Screen {
	...
	private static final ILayerRenderFunction[] LAYERS = {
			// Main layer
			TyrannobookElement::draw,
			// Overlay layer
			TyrannobookElement::drawOverlay };
	...
	private void renderCover(PoseStack matrixStack, Vector3f coverColor) {
		...
		if (!this.book.appearance.title.isEmpty()) {
			...
			fontRenderer.drawShadow(matrixStack, this.book.appearance.title, (this.width / 2F) / scale + 3 - width / 2F, (this.height / 2F - fontRenderer.lineHeight / 2F) / scale - 4, this.book.appearance.coverTextColor);
			matrixStack.popPose();
		}
		...
		if (!this.book.appearance.subtitle.isEmpty()) {
			...
			matrixStack.scale(scale, scale, 1F);
			fontRenderer.drawShadow(matrixStack, this.book.appearance.subtitle, (this.width / 2F) / scale + 7 - width / 2F, (this.height / 2F + 100 - fontRenderer.lineHeight * 2) / scale, this.book.appearance.coverTextColor);
			matrixStack.popPose();
		}
	}
	...
	private void renderPageBackground(PoseStack matrixStack, boolean rightSide) {
		Vector3f pageTint = splitRGB(this.book.appearance.pageTint);
	...
	private void renderPageLayer(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks, List<TyrannobookElement> elements, ILayerRenderFunction layerFunc) {
		RenderSystem.setShaderTexture(0, book.appearance.getCoverTexture());

		Font font = getFontRenderer();

		for (TyrannobookElement element : elements) {
			RenderSystem.setShaderColor(1F, 1F, 1F, 1F);
	...
	public List<TyrannobookElement> getElements(int side) {
