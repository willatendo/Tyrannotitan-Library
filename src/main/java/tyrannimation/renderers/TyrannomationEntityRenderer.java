...
public abstract class TyrannomationEntityRenderer<T extends LivingEntity & IAnimated> extends EntityRenderer<T> implements IAnimatedRenderer<T> {
	static {
		TyrannomationController.addModelFetcher((IAnimated object) -> {
			if (object instanceof Entity) {
				return (IAnimatedModel<Object>) TyrannomationUtils.getTyrannoModelForEntity((Entity) object);
			}
			return null;
		});
	}
	...
	protected TyrannomationEntityRenderer(EntityRendererProvider.Context renderManager, AnimatedModel<T> modelProvider) {
		super(renderManager);
