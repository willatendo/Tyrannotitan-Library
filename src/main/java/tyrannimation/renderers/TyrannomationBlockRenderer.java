...
public abstract class TyrannomationBlockRenderer<T extends BlockEntity & IAnimated> implements BlockEntityRenderer, IAnimatedRenderer<T> {
	static {
		TyrannomationController.addModelFetcher((IAnimated object) -> {
			if (object instanceof BlockEntity) {
				...
				if (renderer instanceof TyrannomationBlockRenderer) {
					return ((TyrannomationBlockRenderer<?>) renderer).getAnimatedModelProvider();
				}
			}
			return null;
		});
	...
	public TyrannomationBlockRenderer(BlockEntityRendererProvider.Context context, AnimatedModel<T> modelProvider) {
