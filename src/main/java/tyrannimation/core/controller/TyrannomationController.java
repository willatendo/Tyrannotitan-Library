package tyrannimation.core.controller;
...
import tyrannimation.core.AnimationEndTypes;
import tyrannimation.core.AnimationState;
import tyrannimation.core.ConstantValue;
...
import tyrannimation.core.builder.TyrannomationBuilder;
import tyrannimation.core.easing.EasingType;
import tyrannimation.core.event.CustomInstructionKeyframeEvent;
import tyrannimation.core.event.ParticleKeyFrameEvent;
import tyrannimation.core.event.SoundKeyframeEvent;
...
import tyrannimation.core.keyframe.KeyFrame;
import tyrannimation.core.keyframe.KeyFrameLocation;
import tyrannimation.core.keyframe.ParticleEventKeyFrame;
import tyrannimation.core.keyframe.TyrannomationPoint;
import tyrannimation.core.keyframe.VectorKeyFrameList;
import tyrannimation.core.processor.IAnimatedBone;
import tyrannimation.core.snapshot.BoneSnapshot;
import tyrannimation.core.util.Axis;

public class TyrannomationController<T extends IAnimated> {
	...
	public interface IAnimationPredicate<P extends IAnimated> {
		AnimationEndTypes test(TyrannomationEvent<P> event);
	}
		...
		if (model != null) {
			if (builder == null || builder.getRawAnimationList().size() == 0) {
				...
				LinkedList<Tyrannomation> animations = new LinkedList<>(builder.getRawAnimationList().stream().map((rawAnimation) -> {
					Tyrannomation animation = finalModel.getAnimation(rawAnimation.animationName, this.animatable);
					if (animation == null) {
	...
	public void process(double tick, TyrannomationEvent event, List<IAnimatedBone> modelRendererList, HashMap<String, Pair<IAnimatedBone, BoneSnapshot>> boneSnapshotCollection, MolangParser parser, boolean crashWhenCantFindBone) {
	...
	protected AnimationEndTypes testAnimationPredicate(TyrannomationEvent<T> event) {
		return this.animationPredicate.test(event);
	}

	private void saveSnapshotsForAnimation(Tyrannomation animation, HashMap<String, Pair<IAnimatedBone, BoneSnapshot>> boneSnapshotCollection) {
