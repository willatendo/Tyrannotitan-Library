package tyrannimation.util.json;
...
public class JsonTyrannomationUtils {
	...
	public static Tyrannomation deserializeJsonToAnimation(Map.Entry<String, JsonElement> element, MolangParser parser) throws ClassCastException, IllegalStateException {
		Tyrannomation animation = new Tyrannomation();
		...
		animation.animationLength = animation_length == null ? null : TyrannomationUtils.convertSecondsToTicks(animation_length.getAsDouble());
		...
		for (Map.Entry<String, JsonElement> bone : bones) {
			AnimatedBone boneAnimation = new AnimatedBone();
	...
	private static double calculateLength(List<AnimatedBone> boneAnimations) {
		double longestLength = 0;
		for (AnimatedBone animation : boneAnimations) {
