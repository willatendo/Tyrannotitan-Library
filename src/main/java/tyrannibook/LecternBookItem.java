...
import core.library.tyrannotitanlib.block.entity.BlockEntityHelper;
	...
	public static void interactWithBlock(PlayerInteractEvent.RightClickBlock event) {
		...
		if (state.is(Blocks.LECTERN)) {
			BlockEntityHelper.get(LecternBlockEntity.class, world, pos).ifPresent(te -> {
				...
				if (!book.isEmpty() && book.getItem() instanceof ILecternBookItem && ((ILecternBookItem) book.getItem()).openLecternScreen(world, pos, event.getPlayer(), book)) {
