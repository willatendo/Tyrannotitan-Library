package tyrannibook.screen.book.element;
...
import tyrannibook.screen.book.TextDataRenderer;
...
public class SelectionElement extends SizedBookElement {
	...
	public void draw(PoseStack stack, int mouseX, int mouseY, float partialTicks, Font fontRenderer) {
		...
		if (hover) {
			fill(stack, this.iconX, this.iconY, this.iconX + IMG_SIZE, this.iconY + IMG_SIZE, this.parent.book.appearance.hoverColor);
		}
		if (unlocked) {
		...
		this.iconRenderer.draw(stack, mouseX, mouseY, partialTicks, fontRenderer);

		if (this.section.parent.appearance.drawSectionListText) {
			String title = this.section.getTitle().replace("\\n", "\n");
			String[] splitTitle = TextDataRenderer.cropStringBySize(title, "", WIDTH + 2, fontRenderer.lineHeight * 2 + 1, fontRenderer, 1F);
			...
			for (int i = 0; i < splitTitle.length; i++) {
				...
				fontRenderer.draw(stack, splitTitle[i], textX, textY, hover ? 0xFF000000 : 0x7F000000);
			}
		}
	}
	...
	public void drawOverlay(PoseStack stack, int mouseX, int mouseY, float partialTicks, Font fontRenderer) {
		if (this.section != null && this.isHovered(mouseX, mouseY)) {
			...
			this.drawTooltip(stack, text, mouseX, mouseY, fontRenderer);
		}
	}
