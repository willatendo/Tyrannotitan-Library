...
public class TextDataRenderer {
	...
	public static String drawText(PoseStack stack, int x, int y, int boxWidth, int boxHeight, TextData[] data, int mouseX, int mouseY, Font fr, BookScreen parent) {
		...
		if (tooltip.size() > 0) {
			parent.renderTooltip(stack, tooltip, Optional.empty(), mouseX, mouseY, fr);
		}
		...
		for (TextData item : data) {
			...
			if (item.useOldColor) {
				...
				if (colFormat != null) {
					...
					LOG.error("Failed to parse color: " + item.color + " for text rendering.");
				}
			}
			...
			boolean mouseInside = (mouseX >= box1X && mouseX <= box1W && mouseY >= box1Y && mouseY <= box1H && box1X != box1W && box1Y != box1H) || (mouseX >= box2X && mouseX <= box2W && mouseY >= box2Y && mouseY <= box2H && box2X != box2W && box2Y != box2H) || (mouseX >= box3X && mouseX <= box3W && mouseY >= box3Y && mouseY <= box3H && box3X != box3W && box1Y != box3H);
	...
	public static String translateString(String s) {
		...
		if (s.indexOf("$(") > s.indexOf(")$") || s.contains(")$")) {
			LOG.error("[Books] [TextDataRenderer] Detected unbalanced localization symbols \"$(\" and \")$\" in string: \"" + s + "\".");
		}
