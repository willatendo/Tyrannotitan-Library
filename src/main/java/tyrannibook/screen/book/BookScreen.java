package tyrannibook.screen.book;
...
public class BookScreen extends Screen {
	...
	public BookScreen(Component title, TyrannobookData book, String page, @Nullable Consumer<String> pageUpdater, @Nullable Consumer<?> bookPickup) {
		super(title);
		this.book = book;
	...
	public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
		...
		if (this.page == -1) {
			this.renderCover(stack, coverColor);
		} else {
			// Jank way to copy last matrix in matrix stack, as no proper way is provided
			PoseStack leftMatrix = new PoseStack();
			leftMatrix.last().pose().multiply(stack.last().pose());
			leftMatrix.last().normal().mul(stack.last().normal());

			PoseStack rightMatrix = new PoseStack();
			rightMatrix.last().pose().multiply(stack.last().pose());
			rightMatrix.last().normal().mul(stack.last().normal());
			...
			renderUnderLayer(stack, coverColor);

			if (renderLeft) {
				renderPageBackground(stack, false);
			}

			if (renderRight) {
				renderPageBackground(stack, true);
			}
	...
	private void renderCover(PoseStack stack, Vector3f coverColor) {
		Font fontRenderer = getFontRenderer();
		...
		blit(stack, centerX, centerY, 0, 0, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE);
		RenderSystem.setShaderColor(1f, 1f, 1f, 1f);

		if (!this.book.appearance.title.isEmpty()) {
			blit(stack, centerX, centerY, 0, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE);
			...
			fontRenderer.drawShadow(stack, this.book.appearance.title, (this.width / 2F) / scale + 3 - width / 2F, (this.height / 2F - fontRenderer.lineHeight / 2F) / scale - 4, this.book.appearance.getCoverTextColor());
			stack.popPose();
		}
		...
		if (!this.book.appearance.subtitle.isEmpty()) {
			...
			stack.scale(scale, scale, 1F);
			fontRenderer.drawShadow(stack, this.book.appearance.subtitle, (this.width / 2F) / scale + 7 - width / 2F, (this.height / 2F + 100 - fontRenderer.lineHeight * 2) / scale, this.book.appearance.getCoverTextColor());
			stack.popPose();
		}
	}

	private void renderUnderLayer(PoseStack stack, Vector3f coverColor) {
	...
	private void renderPageBackground(PoseStack stack, boolean rightSide) {
	...
	private void renderPageLayer(PoseStack stack, int mouseX, int mouseY, float partialTicks, List<BookElement> elements, ILayerRenderFunction layerFunc) {
		RenderSystem.setShaderTexture(0, book.appearance.getCoverTexture());
		...
		for (BookElement element : elements) {
			RenderSystem.setShaderColor(1F, 1F, 1F, 1F);
			layerFunc.draw(element, stack, mouseX, mouseY, partialTicks, font);
		}
	}
