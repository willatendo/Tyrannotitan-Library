...
import tyrannibook.data.element.TextComponentData;

public class TextComponentDataRenderer {
	public static String drawText(PoseStack stack, int x, int y, int boxWidth, int boxHeight, TextComponentData[] data, int mouseX, int mouseY, Font fr, List<Component> tooltip) {
		String action = "";
		...
		for (TextComponentData item : data) {
			...
			for (int lineNumber = 0; lineNumber < textLines.size(); ++lineNumber) {
				...
				drawScaledTextComponent(stack, fr, textComponent, atX, atY, item.dropShadow, item.scale);
	 ...
	 *
	 * @param stack   the given matrix stack used for rendering.
	 ...
	 */
	public static void drawScaledTextComponent(PoseStack stack, Font font, FormattedText textComponent, float x, float y, boolean dropShadow, float scale) {
		stack.pushPose();
		stack.translate(x, y, 0);
		stack.scale(scale, scale, 1F);

		if (dropShadow) {
			font.drawShadow(stack, Language.getInstance().getVisualOrder(textComponent), 0, 0, 0);
		} else {
			font.draw(stack, Language.getInstance().getVisualOrder(textComponent), 0, 0, 0);
		}
